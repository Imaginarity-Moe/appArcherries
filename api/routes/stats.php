...
function handle_stats(string $method, string $path): void
{
    ...
    if ($sub === '/heatmap' || $sub === '/heatmap/targets') {
        if ($sub === '/heatmap/targets') stats_heatmap_targets($user['id']);
        else stats_heatmap($user['id']);
        return;
    }
...
/**
 * Liste aller Tier-/Auflage-Namen, für die der User bereits Treffer mit
 * normalisierten Koordinaten gespeichert hat. Wird als Dropdown im Frontend genutzt.
 */
function stats_heatmap_targets(int $user_id): void
{
    $stmt = db()->prepare(
        'SELECT tt.animal_or_face AS name,
                COUNT(s.id)        AS shot_count
         FROM training_targets tt
         JOIN trainings t ON t.id = tt.training_id
         JOIN shots s ON s.target_id = tt.id
         WHERE t.user_id = ?
           AND tt.animal_or_face IS NOT NULL AND tt.animal_or_face <> ""
           AND s.x_norm IS NOT NULL AND s.y_norm IS NOT NULL
         GROUP BY tt.animal_or_face
         ORDER BY shot_count DESC, tt.animal_or_face ASC'
    );
    $stmt->execute([$user_id]);
    $rows = $stmt->fetchAll();
    res_json([
        'targets' => array_map(fn ($r) => [
            'name'       => $r['name'],
...
/**
 * Aggregierte Shots für ein bestimmtes Tier/Face. Liefert die rohen
 * (x, y, points, zone, discipline)-Tupel und etwas Meta (Total, Avg).
 *
 * Query-Params:
 *   target       (required) – animal_or_face exakt
 *   distance     (optional) – distance_m exakt
 *   discipline   (optional) – training-discipline
 *   bow_type     (optional) – training-bow_type
 */
function stats_heatmap(int $user_id): void
{
    $target     = req_query('target');
    if ($target === null || $target === '') res_error('target erforderlich');

    $where = ['t.user_id = ?', 'tt.animal_or_face = ?', 's.x_norm IS NOT NULL', 's.y_norm IS NOT NULL'];
    $args  = [$user_id, $target];

    $distance   = req_query('distance');
    ...
    $sql = 'SELECT s.x_norm, s.y_norm, s.points, s.zone, t.discipline
            FROM shots s
    ...
    $total  = count($shots);
    $scored = array_filter($shots, fn ($s) => $s['points'] !== null);
    $avg    = $scored ? array_sum(array_column($scored, 'points')) / count($scored) : 0;

    // Distinct Distanzen für die Filter-UI
    $distStmt = db()->prepare(
        'SELECT DISTINCT tt.distance_m
         FROM training_targets tt
         JOIN trainings t ON t.id = tt.training_id
         WHERE t.user_id = ? AND tt.animal_or_face = ?
           AND tt.distance_m IS NOT NULL
         ORDER BY tt.distance_m ASC'
    );
    $distStmt->execute([$user_id, $target]);
    $distances = array_map(fn ($r) => (int)$r['distance_m'], $distStmt->fetchAll());
