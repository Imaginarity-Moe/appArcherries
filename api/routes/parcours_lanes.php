 ...
 *  POST   /parcours/<id>/lanes                  → Bahn anlegen / upserten via lane_number
...
function handle_parcours_lanes(string $method, int $user_id, int $parcours_id, string $rest): void
{
    ...
    // /lanes
    if ($rest === '' || $rest === '/') {
        if ($method === 'GET') { lanes_list($parcours_id); return; }
        if ($method === 'POST') {
            if (!$owned) res_error('Nur der Parcours-Owner kann Bahnen anlegen', 403);
            lanes_upsert($parcours_id);
            return;
        }
    ...
    if (preg_match('#^/(\d+)$#', $rest, $m)) {
        ...
        if ($method === 'PATCH' || $method === 'DELETE') {
            ...
            if ($method === 'PATCH') lanes_update($parcours_id, $lid);
    ...
    if (preg_match('#^/(\d+)/image$#', $rest, $m)) {
        ...
        if ($method === 'POST')   { lanes_image_upload($parcours_id, $lid); return; }
        if ($method === 'DELETE') { lanes_image_delete($parcours_id, $lid); return; }
        res_error('Method not allowed', 405);
    }
...
function lanes_list(int $parcours_id): void
{
    $stmt = db()->prepare(
    ...
    $rows = array_map('lane_serialize', $stmt->fetchAll());
    res_json(['lanes' => $rows]);
}

function lanes_upsert(int $parcours_id): void
{
    $in = req_json();
    ...
    lane_detail($parcours_id, $lid, 201);
}

function lanes_update(int $parcours_id, int $lid): void
{
    $in = req_json();
    $sets = [];
    $vals = [];
...
function lanes_image_upload(int $parcours_id, int $lid): void
{
    $stmt = db()->prepare('SELECT image_path FROM parcours_lanes WHERE id = ? AND parcours_id = ?');
    $stmt->execute([$lid, $parcours_id]);
    $row = $stmt->fetch();
...
function lanes_image_delete(int $parcours_id, int $lid): void
{
    $stmt = db()->prepare('SELECT image_path FROM parcours_lanes WHERE id = ? AND parcours_id = ?');
    $stmt->execute([$lid, $parcours_id]);
    $row = $stmt->fetch();
...
function lane_detail(int $parcours_id, int $lid, int $status = 200): void
{
    $stmt = db()->prepare(
        'SELECT id, parcours_id, lane_number, animal_description,
                distance_blue, distance_red, distance_yellow, distance_white,
    ...
    $row = $stmt->fetch();
    if (!$row) res_error('Not found', 404);
    res_json(['lane' => lane_serialize($row)], $status);
}

function lane_serialize(array $r): array
{
    return [
        'id'                 => (int)$r['id'],
